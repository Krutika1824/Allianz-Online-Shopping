...
console.log(price+""+document.getElementById("quantity"+product_id));
...
<a href="http://jigsaw.w3.org/css-validator/check/referer"><img style="border:0;width:88px;height:31px"  src="http://jigsaw.w3.org/css-validator/images/vcss-blue" alt="Valid CSS!" vspace="8" border="0" /></a>
        ...
        <div id="templatemo_content_right">
           ...
           <%--      <ul>
                    <li><a>${prod.id}</a></li>
                    <li><a>${prod.code}</a></li>
                    <li><a>${prod.price}</a></li>
                    <li><a>${prod.description}</a></li>
                    <li><a>${prod.manfDate}</a></li>
            </ul> --%>
            <%-- <input type="text" name="quantity${product.id}" value="">  --%>
           <td>
          ...
          <input type="text" name="quantity${product.id}" id="quantity${product.id}" value="1" onblur="calculatePrice(${product.id})"/></td>
          <td>
           <input type="hidden" name="price${product.id}" id="price${product.id}" value="${product.price}"/>
           
          <input type="text" name="total${product.id}" id="total${product.id}" value="${product.price}" onblur="calculatePrice(${product.id})"/></td>
          
           
           
           <div class="buy_now_button"><a href="ProductController?action=deletefromCart&index=${status.index}">Remove</a></div>
           <div class="buy_now_button"><button type="submit">Checkout</button></div>
